  ...
  return `
    <div class="candidate">
      <div class="candidate-top">
        <strong>${title}</strong>
        <span class="pill ${pill(status)}">${human(status)}</span>
      </div>
      <p>${detail}</p>
    </div>`;
}
...
function renderLifecycle(data) {
  ...
  qs("lifecycle").innerHTML = steps.map(([name, detail, state]) =>
    `<div class="lifecycle-step ${state}"><strong>${name}</strong><span>${detail}</span></div>`
  ).join("");
}
...
function renderIncident(data) {
  qs("candidate-list").innerHTML = data.incident.recovery_candidates.map((item) =>
    candidateRow(
      human(item.action_type),
      item.status,
      `${human(item.recovery_class)} | recovery evidence: ${item.recovery_evidence_event_id ? "present" : "missing"} | verification evidence: ${item.verification_evidence_event_id ? "present" : "missing"}`
    )
  ).join("");

  const outcomes = data.incident.recovery_outcomes;
  qs("side-effect-list").innerHTML = data.incident.side_effects.map((item) =>
    candidateRow(
      human(item.action_type),
      outcomes[item.action_type] || item.recovery_class,
      `${human(item.recovery_class)} | resources: ${(item.resource_keys || []).join(", ") || "none"}`
    )
  ).join("");
  ...
  text(
    "state-memory",
    data.incident.after_recovery.memory["sales:last_instruction"] ?? "cleared"
  );
...
function renderAssessment(data) {
  ...
  qs("remediation-list").innerHTML = a.remediation.length
    ? a.remediation.map((item) =>
        candidateRow(
          `${item.priority} ${human(item.kind)}`,
          item.priority,
          human(item.evidence)
        )
      ).join("")
    : candidateRow("No derived remediation", "verified", "Current evidence has no remediation blockers.");

  qs("claim-list").innerHTML = a.claim_limits
    .map((item) => `<span class="claim">${human(item)}</span>`)
    .join("");
}
...
async function load() {
  ...
  try {
    ...
    if (data.product.authority !== "none") throw new Error("UI fixture must remain non-authorizing");
    renderOverview(data);
    renderIncident(data);
    renderAssessment(data);
  } catch (error) {
    console.error(error);
    qs("error-banner").hidden = false;
  }
}
