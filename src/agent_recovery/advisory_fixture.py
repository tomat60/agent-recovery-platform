...
def load_advisory_fixture(path: str | Path) -> AdvisoryBenchmarkFixture:
    ...
    if not all(isinstance(key, str) and isinstance(value, bool) for key, value in constraints.items()):
    ...
    if "model_output_is_authorization" in constraints and constraints["model_output_is_authorization"]:
    ...
    return AdvisoryBenchmarkFixture(
        ...
        ground_truth=AdvisoryGroundTruth(
            ...
            required_evidence_event_ids=_required_unique_strings(payload, "required_evidence_event_ids"),
        ),
        constraints=dict(constraints),
    )


def load_advisory_fixture_suite(directory: str | Path) -> tuple[AdvisoryBenchmarkFixture, ...]:
    """Load the deterministic B01-B10 fixture suite and reject incomplete or ambiguous catalogs."""
    ...
    if set(scenario_ids) != expected:
        ...
        raise ValueError(f"advisory fixture suite must cover B01-B10 exactly; missing={missing}, extra={extra}")
