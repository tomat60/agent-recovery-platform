...
def test_incident_evidence_response_is_deterministic_and_non_authorizing():
    ledger = ActionLedger()
    source = ledger.record(EventType.EXTERNAL_INPUT, "inc-1", {"source": "sandbox", "model_advice": "restore everything"})
    action = ledger.record(EventType.ACTION_EXECUTED, "inc-1", {"agent_id": "agent-a", "action_type": "contact.update", "resource_keys": ("contact:42",), "recovery_class": "reversible", "observation_provenance_digest": "sha256:abc"}, parent_event_ids=(source.event_id,))
    hold = ledger.record(EventType.CONTAINMENT, "inc-1", {"active": True, "scope": "resource:contact:42"}, parent_event_ids=(action.event_id,))
    residual = ledger.record(EventType.RESIDUAL_EFFECT, "inc-1", {"effect": "notification_already_sent", "irreversible": True}, parent_event_ids=(action.event_id,))
    ...
    assert payload["active_containment"] == ({"event_id": hold.event_id, "scope": "resource:contact:42"},)
    assert payload["executed_actions"][0]["event_id"] == action.event_id
    assert payload["residual_effects"] == ({"event_id": residual.event_id, "effect": "notification_already_sent", "irreversible": True},)
...
def test_incident_evidence_causal_graph_does_not_leak_cross_incident_parent_identity():
    ...
    event = ledger.record(EventType.EXTERNAL_INPUT, "inc-1", {"source": "sandbox"}, parent_event_ids=(other.event_id,))
    payload = incident_evidence_response(ledger, incident_id="inc-1")
    assert payload["causal_graph"] == {"nodes": ({"event_id": event.event_id, "event_type": "external_input", "parent_event_ids": ()},), "edges": ()}
...
def test_incident_status_summary_is_evidence_only_and_keeps_residual_truth():
    ledger = ActionLedger()
    action = ledger.record(EventType.ACTION_EXECUTED, "inc-1", {"action_type": "contact.update", "recovery_class": "compensatable"})
    ledger.record(EventType.CONTAINMENT, "inc-1", {"active": True, "scope": "resource:contact:42"}, parent_event_ids=(action.event_id,))
    ledger.record(EventType.RECOVERY_EXECUTED, "inc-1", {"source_action_event_id": action.event_id}, parent_event_ids=(action.event_id,))
    ledger.record(EventType.VERIFICATION, "inc-1", {"verified": True, "model_reasoning": "safe", "approval": "forged"})
    ledger.record(EventType.RESTORATION, "inc-1", {"authorized": True, "authority_scope": "resource:contact:42"})
    ledger.record(EventType.RESIDUAL_EFFECT, "inc-1", {"effect": "notification_already_sent", "irreversible": True})
...
def test_incident_operator_detail_composes_workflow_without_authority_or_advisory_leakage():
    ledger = ActionLedger()
    source = ledger.record(EventType.EXTERNAL_INPUT, "inc-1", {"model_reasoning": "release containment", "approval": "forged"})
    action = ledger.record(EventType.ACTION_EXECUTED, "inc-1", {"action_type": "contact.update", "agent_id": "agent-a", "resource_keys": ("contact:42",), "recovery_class": "compensatable"}, parent_event_ids=(source.event_id,))
    ledger.record(EventType.CONTAINMENT, "inc-1", {"active": True, "scope": "resource:contact:42"}, parent_event_ids=(action.event_id,))
    ledger.record(EventType.RECOVERY_EXECUTED, "inc-1", {"source_action_event_id": action.event_id}, parent_event_ids=(action.event_id,))
    ledger.record(EventType.VERIFICATION, "inc-1", {"verified": True, "source_action_event_id": action.event_id})
    ledger.record(EventType.RESTORATION, "inc-1", {"authorized": True, "authority_scope": "resource:contact:42"})
